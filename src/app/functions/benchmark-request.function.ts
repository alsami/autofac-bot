import {
  ...
  notifyBenchmarkRequestFailure,
  notifyBenchmarkResults,
  notifyInvalidForIssue,
  notifyPullRequestInfoRequestFailure,
  notifyRepositoryInfoRequestFailure,
} from '@autofac-bot/functions';
import {
  BenchmarkRequest,
  RunningRequest,
  TargetSourceRepository,
} from '@autofac-bot/models';
import { validBenchmark, validRequest } from '@autofac-bot/validation';
import fetch, { Response } from 'node-fetch';
import { Context } from 'probot';

let runningRequest: RunningRequest | null = null;

export async function postBenchmarkRequest(
  context: Context,
  words: string[]
): Promise<void> {
  if (runningRequest) {
  ...
  let targetSourceRepository = await extractTargetAndSourceRepository(
  ...
  const benchmarkRequest = await buildRequest(
    words,
    targetSourceRepository,
...
async function extractTargetAndSourceRepository(
